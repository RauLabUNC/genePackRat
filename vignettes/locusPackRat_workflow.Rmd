...
Gene mode is designed for analyzing gene lists from RNA-seq, QTL mapping, or other gene-centric studies.
...
motif_data <- data.table(
  ...
  start = sample(regions$start, 120, replace = TRUE),
  end = sample(regions$end, 120, replace = TRUE),
...
makeGeneSheet(
  ...
  split_criteria = list(
    ...
    "With_Mouse_Ortholog" = "!is.na(mouse_symbol)"
  ),
  include_supplementary = TRUE,
  project_dir = "cancer_genes"
)
...
listPackRatTables("rnaseq_analysis")
listPackRatTables("atacseq_analysis")
listPackRatTables("cancer_genes")
```
...
# Advanced Filtering with filterGenes()

The `filterGenes()` function provides powerful relational filtering capabilities for working with genomic data. It can be used with data loaded from your locusPackRat projects to perform complex filtering operations.

## Basic Filtering

Filter genes or regions based on conditions in your data:

```{r basic_filter, eval=FALSE}
# Load processed genes from project
genes <- fread("rnaseq_analysis/.locusPackRat/input/genes.csv")

# Filter for significant differential expression
significant_genes <- filterGenes(
  genes,
  filters = list(
    makeFilter("padj", "<", 0.05),
    makeFilter("abs(log2FC)", ">", 2)
  )
)

# Filter for specific chromosomes
chr_subset <- filterGenes(
  genes,
  filters = list(
    makeFilter("chr", "%in%", c("1", "2", "3", "X"))
  )
)
```
...
Use reference tables to include or exclude genes:

```{r relational_filter, eval=FALSE}
# Load supplementary data
tissue_expr <- fread("rnaseq_analysis/.locusPackRat/supplementary/tissue_expression.csv")
pathways <- fread("rnaseq_analysis/.locusPackRat/supplementary/pathway_annotations.csv")

# Keep only genes with brain-specific expression
brain_genes <- filterGenes(
  genes,
  ref_data = tissue_expr,
  join_type = "semi",
  join_by = c("gene_symbol" = "gene_symbol"),
  filters = list(
    makeFilter("max_tissue", "==", "brain"),
    makeFilter("tissue_specific", "==", TRUE)
  )
)

# Exclude genes in immune pathways
non_immune <- filterGenes(
  genes,
  ref_data = pathways,
  join_type = "anti",
  join_by = c("gene_symbol" = "gene_symbol"),
  filters = list(
    makeFilter("pathway", "==", "Immune response")
  )
)

# Add pathway information to genes (left join)
genes_with_pathways <- filterGenes(
  genes,
  ref_data = pathways,
  join_type = "left",
  join_by = c("gene_symbol" = "gene_symbol")
...
The `makeFilter()` function supports various comparison operators:

```{r complex_filters, eval=FALSE}
# Numeric comparisons
high_expr <- makeFilter("baseMean", ">", 1000)
mid_range <- makeFilter("log2FC", "between", c(-1, 1))

# String matching
mouse_genes <- makeFilter("gene_symbol", "matches", "^[A-Z][a-z]+[0-9]*$")
human_genes <- makeFilter("gene_symbol", "matches", "^[A-Z0-9]+$")

# Set membership
cancer_genes <- makeFilter("gene_symbol", "%in%", c("Tp53", "Myc", "Kras", "Pten"))
not_mitochondrial <- makeFilter("chr", "%nin%", c("MT", "M"))

# NA handling
has_ortholog <- makeFilter("mouse_symbol", "not_na")
missing_data <- makeFilter("padj", "is_na")

# Combine multiple filters
complex_filter <- filterGenes(
  genes,
  filters = list(
    makeFilter("chr", "%in%", c("1", "2", "3")),
    makeFilter("padj", "<", 0.01),
    makeFilter("abs(log2FC)", ">", 1.5),
    makeFilter("baseMean", ">", 100)
  )
)
```

## Integration with makeGeneSheet()

Use filterGenes() to pre-process data before generating output:

```{r filter_then_export, eval=FALSE}
# Filter genes first
...
# Save filtered results back to project
fwrite(filtered_genes, "rnaseq_analysis/.locusPackRat/input/genes_filtered.csv")

# Generate Excel with filtered data
makeGeneSheet(
  format = "excel",
  output_file = "tissue_specific_DEGs.xlsx",
  filter_expr = "tissue_specific == TRUE",
  include_supplementary = TRUE,
  project_dir = "rnaseq_analysis"
)
```

## Working with Region Data

filterGenes() also works with genomic regions from ATAC-seq or ChIP-seq:

```{r filter_regions, eval=FALSE}
# Load regions
regions <- fread("atacseq_analysis/.locusPackRat/input/regions.csv")
chromatin <- fread("atacseq_analysis/.locusPackRat/supplementary/chromatin_states.csv")

# Filter for active regulatory regions
active_regions <- filterGenes(
  regions,
  ref_data = chromatin,
  join_type = "semi",
  join_by = c("chr" = "chr", "start" = "start", "end" = "end"),
  filters = list(
    makeFilter("state", "%in%", c("Active_Promoter", "Strong_Enhancer"))
  )
)

# High-confidence peaks with TF binding
tf_motifs <- fread("atacseq_analysis/.locusPackRat/supplementary/tf_motifs.csv")
high_conf_peaks <- filterGenes(
  regions,
  ref_data = tf_motifs,
  join_type = "semi",
  join_by = c("chr" = "chr", "start" = "start", "end" = "end"),
  filters = list(
    makeFilter("fold_enrichment", ">", 10),
    makeFilter("qvalue", "<", 1e-10),
    makeFilter("motif_score", ">", 12)
  )
)
```
...
- Use `semi` join for efficient filtering when you only need to check existence
- Use `anti` join to exclude records matching criteria
- Chain multiple filterGenes() calls for complex multi-step filtering
- Consider saving intermediate results for reuse
